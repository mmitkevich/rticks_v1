...
cfg <- config(backtest) %>% modifyList(list(
  ...
  log_level = LOG$INFO,
  log_stdout = LOG$WARN,
  zero_position_freq= as.numeric(months(3)), #as.numeric(months(2)),
...
params <- data_frame(
  # limits
  limit.buy     = 23,  # buy when price <= buy only.  NA. +Inf = buy always.  -Inf = buy never
  stop.buy      = 19,    # FIXME: no buy lower than 18
  risk.buy      = 15,

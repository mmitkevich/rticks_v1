...
cfg <- config(backtest) %>% modifyList(list(
  ...
  mpi=0.05, # overrides spread's
  multiplier=1 # overrides spread's
))
...
params <- data_frame(
  ...
  limit.buy     = 28,    # buy when price <= buy only.  NA. +Inf = buy always.  -Inf = buy never
  stop.buy      = 25,
  ...
  currency       = NA, # could specify virtual_id of the tracking currency

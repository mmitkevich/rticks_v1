...
outdir <- "~/r-tools/PortfolioOptimization/Close6M/ReturnsData/"

params <- data_frame(
  # limits
  limit.buy     = 50,  # buy when price <= buy only.  NA. +Inf = buy always.  -Inf = buy never
  ...
  symbol        = "CL.NYMEX",   # exante prefix of contract series
...
returns.xts <- r$metrics %>%
  select(datetime, rtn) %>%
  write.csv(file = paste0(outdir, r$metrics$symbol[1]), row.names = F)

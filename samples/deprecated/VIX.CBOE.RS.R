...
stop  <- as_datetime("2017-06-01")

params <- data_frame(
  # limits
  limit.buy     = 0.35,  # buy when price <= buy only.  NA. +Inf = buy always.  -Inf = buy never
  stop.buy      = 0.0,    # FIXME: no buy lower than 18
  risk.buy      = -1.0,
